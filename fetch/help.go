...
func Help(options config.Options, version string) {
	...
	if util.Member(options.Group, "display,more,docs") {
		...
		fmt.Println("     --multi-line     Handles multiple logs entries in one Event Message")
	...
	if util.Member(options.Group, "advanced,more,docs") {
		...
		fmt.Println("     --sortkey        Add each line with a sortable time based key [default: false]")
		fmt.Println("     --refresh        Tail Refresh interval in seconds [default: 5]")
		fmt.Println("     --region         Override or set the AWS Region")
	...
	if util.Member(options.Group, "env,environment,more,docs") {
		...
		fmt.Println("  LAWSG_MULTI_LINE     Handles multiple logs entries in one Event Message")
		...
		fmt.Println("  LAWSG_SORTKEY        Add each line with a sortable time based key [default: false]")
		fmt.Println("  LAWSG_REFRESH        Tail Refresh interval in seconds [default: 5]")
		fmt.Println("  LAWSG_REGION         Override or set the AWS Region")
		fmt.Println("  LAWSG_ENV            Environment prefix multiple AWS ENV Vars (ex: STAGING_AWS_REGION)")
